const $ = new Env('鸿星尔克'); // 适配青龙面板
const notify = $.isNode() ? require('./sendNotify') : ''; // 引用通知模块
...
if (typeof $request !== 'undefined') {
    ...
    !(async () => {
        console.log('🎉 [青龙环境] 开始执行任务');
        const requestData = loadRequestData();
        if (!requestData) {
            console.error('❌ 未找到有效的请求数据，请检查环境变量 erke_data');
            return;
        }
...
function loadRequestData() {
    ...
    if (data) {
        ...
        console.error('❌ 未找到有效的 URL 数据，请配置青龙环境变量 erke_data');
        return null;
    }
}
