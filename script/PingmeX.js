...
async function handleTask() {
    ...
    if (!ids.length) {
        $.log(`\n⚠️ 未能识别有效账号。环境变量内容详情: ${JSON.stringify(store)}`);
        $.msg($.name, '⚠️ 无账号数据', '请先打开 PingMe 触发抓包并同步');
        return;
    }
    ...
    const fetchApi = (path) => {
        ...
        return new Promise(resolve => {
            $.get({ url, headers }, (err, resp, body) => {
                ...
                catch (e) { resolve({ retcode: -1, retmsg: '接口响应解析失败' }); }
            });
        });
    };
...
function loadStore() {
    ...
    try { 
        ...
        // 2. 核心补全：如果没 order 字段但有 accounts，自动生成
