...
if ($.isNode()) {
    try {
        ...
        $.log('⚠️ 未找到 sendNotify.js，青龙推送将失效');
    }
}
...
async function handleCapture() {
    ...
    if (!store.order.includes(fp)) store.order.push(fp);
    
    const dataStr = JSON.stringify(store);
    $.setdata(dataStr, storeKey);

    $.msg($.name, existed ? '🔄 账号已更新' : '✅ 新账号入库', `${alias}（id:${fp}）`);
...
async function handleTask() {
    ...
    // ============ 关键修改：任务结束后触发推送 ============
    if ($.isNode() && notify) {
        await notify.sendNotify(`${$.name} 执行完毕`, allResults.join('\n\n'));
    } else {
        $.msg($.name, `全部任务完成 (${ids.length}个账号)`, allResults.join('\n\n'));
    }
}
    ...
    const fetchApi = (path) => {
        ...
        return new Promise(resolve => {
            $.get({ url, headers }, (err, resp, body) => {
                ...
                catch (e) { resolve({ retcode: -1, retmsg: '接口解析失败' }); }
            });
        });
    };

    let res = await fetchApi('queryBalanceAndBonus');
    if (res && res.retcode === 0) msgs.push(`💰 余额：${res.result.balance} Coins`);
...
async function syncToQingLong(value) {
    try {
        ...
        if (env) {
            await QL.updateEnv({ value, name: storeKey, remarks: "QX自动同步", id: env.id });
            $.log(`✅ 青龙环境变量更新成功`);
        } else {
...
function loadStore() {
    let raw = "";
    if ($.isNode()) {
        raw = process.env[storeKey] || "";
    } else {
        raw = $.getdata(storeKey) || "";
    }
    ...
    try { 
        ...
        if (!store.order || !Array.isArray(store.order)) store.order = Object.keys(store.accounts);
        return store;
    } catch (e) { 
        $.log(`❌ 数据解析异常: ${e.message}`);
