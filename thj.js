...
class UserInfo {
    constructor(user) {
        ...
        this.headers = {
            ...
            'Referer': `https://servicewechat.com/wx7fc1c39c9cf402fd/508/page-frame.html`,
            'Token': this.token,
            'Api-Version': `v1.0`,
...
// 获取Cookie（重写容错，修复token.split报错）
async function getCookie() {
    try {
        if ($request && $request.method === 'OPTIONS') return;
        const headers = ObjectKeys2LowerCase($request.headers);
        const Body = $.toObj($response.body, {});
        let res = decrypt(Body?.info, Body?.bl_input_random_string);
        //修复token空分割报错
        if (!headers?.token) throw new Error("请求头无Token");
        const tokenStr = headers.token;
        let [, token] = tokenStr.split(" ");
        if (!token) throw new Error("Token解析失败");
        const mobile = res?.data?.mobile;
        if (!mobile) throw new Error("未获取手机号");
